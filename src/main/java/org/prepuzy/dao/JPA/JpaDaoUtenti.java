...
public class JpaDaoUtenti implements DaoUtenti {
	...
	try {
	    TypedQuery<AbilitaFrutto> query = em.createQuery(
		    "SELECT a FROM AbilitaFrutto a JOIN a.visibileAPersonaggio p JOIN a.frutto fr WHERE p.id = :idPersonaggio AND fr.id = idFrutto",
		    AbilitaFrutto.class).setParameter("ididPersonaggio", personaggio.getId())
		    .setParameter("idFrutto", idFrutto);
	...
	try {
		...
		TypedQuery<AbilitaProfessione> query = em.createQuery(
			"SELECT a FROM AbilitaProfessione a JOIN a.visibileAPersonaggio p JOIN a.professione pr WHERE p.id = :idPersonaggio AND pr.id = :idProfessione",
			AbilitaProfessione.class).setParameter("idPersonaggio", personaggio.getId())
			.setParameter("idProfessione", idProfessione);
		return query.getResultList();
	    } else {
	...
	EntityManager em = JpaDaoFactory.getEntityManager();
	try {
	    TypedQuery<Personaggio> query = em
		    .createQuery("SELECT p FROM Personaggio p WHERE p.utente.id = :idUtente", Personaggio.class)
