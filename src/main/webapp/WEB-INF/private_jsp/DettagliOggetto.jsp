...
<body>
	<nav>
		...
		<div id="menuToggle"><input type="checkbox" /> <span></span> <span></span> <span></span>
			<ul id="menu">
				...
				<li><a href="${pageContext.request.contextPath}/master/AbilitaoggettoServlet">Abilita Frutti</a></li>
	...
	<div class="container">
		...
		<div class="centerBar">
			...
			%>
			<h1>Dettagli di <%=oggetto.getNome()%></h1>

			<div class="dettagliOggetto">
				<p><strong>Nome:</strong> <%=oggetto.getNome()%></p>
				<p><strong>Descrizione:</strong> <%=oggetto.getDescrizione()%></p>
				<p><strong>Prezzo:</strong> <%=oggetto.getPrezzo()%> berry</p>
				<p><strong>Peso:</strong> <%=oggetto.getPeso()%></p>
				...
				<table>
					...
					<tr>
						...
						<td><%=((oggetto.getForza() - 10) / 2 >= 0) ? "+" + ((oggetto.getForza() - 10) / 2) : ((oggetto.getForza() - 10) / 2)%></td>
				</tr>
					<tr>
						<td>Destrezza:</td>
	...
	: ((oggetto.getCarisma() - 10) / 2)%></td>
				...
				<ul>
					...
					%>
					<li><a href="DettagliResistenzaServlet?idResistenza=<%=resistenza.getId()%>"><%=resistenza.getNome()%></a></li>
					<%
					}
					} else {
					%>
				...
				<ul>
					...
					%>
					<li><a href="DettagliProfessioneServlet?idProfessione=<%=professione.getId()%>"><%=professione.getNome()%></a></li>
					<%
					}
					} else {
					%>
				...
				<ul>
					...
					%>
					<li><a href="DettagliStatusAlteratiServlet?idStatusAlterato=<%=status.getId()%>"><%=status.getNome()%></a></li>
					<%
					}
					} else {
					%>
				...
				<div class="actionButtons">
					<form action="${pageContext.request.contextPath}/master/ModificaOggettoServlet" method="get" style="display: inline;"><input type="hidden" name="idOggetto" value="<%=oggetto.getId()%>">
						<button type="submit" class="buttonMod">Modifica Oggetto</button></form>
					<form action="${pageContext.request.contextPath}/master/EliminaOggettoServlet" method="post" style="display: inline;" onsubmit="return confirm('Sei sicuro di voler cancellare questo oggetto?');">
