...
<body>
 ...
 if (utenteLoggato.getRole().equals(Role.MASTER))
     isMaster = true;
 Personaggio mercante = (Personaggio) request.getAttribute("mercante");
 if (mercante != null) {
 %>
			<h2>Mercante: <%=mercante.getNome()%></h2>
			<h3>Oggetti in vendita:</h3>
			<form action="DettagliMercanteServlet" method="post"><input type="hidden" name="idMercante" value="<%=mercante.getId()%>">
				<table>
					<tr>
						<th>Oggetto</th>
						<th>Prezzo</th>
					...
					%>
					<tr>
						<td><a href="${pageContext.request.contextPath}/DettagliOggettoServlet?id=<%=invOggetto.getId()%>"><%=invOggetto.getOggetto().getNome()%></a></td>
