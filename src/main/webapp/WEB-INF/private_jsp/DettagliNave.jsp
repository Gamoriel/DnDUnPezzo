...
<body>
	...
	<div class="container">
		...
		<div class="centerBar">
			...
			if (nave != null) {
			%>
			<h2><%=nave.getNome()%></h2> <img src="<%=nave.getImmagine()%>" alt="Immagine Nave" class="jollyRogerImage">
			<h2>Statistiche</h2>
			<table>
				<tr>
					<th>Statistica</th>
 ...
 if (inventario != null) {
 %>
			<form action="${pageContext.request.contextPath}/ModificaBerryNaveServlet" method="post"><label for="berry">Berry:</label> <input type="number" id="berry" name="berry"
				value="<%=inventario.getBerry()%>" min="0" required> <input type="hidden" name="idNave" value="<%=nave.getId()%>">
				<button type="submit" class="buttonMod">Aggiorna Berry</button></form> <%
 }
 %>
			<h3>Aggiungi Oggetto all'Equipaggiamento</h3>
			<form action="${pageContext.request.contextPath}/ModificaEquipaggiamentoNaveServlet" method="post"><input type="hidden" name="idNave" value="<%=nave.getId()%>"> <select
				name="idOggetto">
			...
			<form action="${pageContext.request.contextPath}/RimuoviOggEquipNaveServlet" method="post"><input type="hidden" name="idNave" value="<%=nave.getId()%>"> <select name="idOggetto">
 ...
 %>
			<div class="actionButtons">
				<form action="${pageContext.request.contextPath}/master/ModificaNaveServlet" method="get"><input type="hidden" name="idNave" value="<%=nave != null ? nave.getId() : ""%>">
					<button type="submit" class="buttonMod">Modifica Nave</button></form>

				<form action="${pageContext.request.contextPath}/master/EliminaNaveServlet" method="post"><input type="hidden" name="idNave" value="<%=nave != null ? nave.getId() : ""%>">
					<button type="submit" class="buttonDel" onclick="return confirm('Sei sicuro di voler eliminare questa nave?');">Elimina Nave</button></form>
		</div>
	</div>
</div>
</body>
