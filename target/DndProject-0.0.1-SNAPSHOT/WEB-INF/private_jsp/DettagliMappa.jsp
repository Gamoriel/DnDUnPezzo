...
<body>
    ...
    <div class="container">
        ...
        %>
        <h2><%=mappa.getNome()%></h2>
        <img src="<%=mappa.getImmagine()%>" alt="Immagine della Mappa" class="mappaImage">
        <p><strong>Descrizione:</strong> <%=mappa.getDescrizione()%></p>

        <!-- Elenco Personaggi Associati -->
        ...
        <!-- Bottoni per Modifica e Cancellazione -->
        <div class="actionButtons">
            ...
            <form action="EliminaMappaServlet" method="post" onsubmit="return confirm('Sei sicuro di voler cancellare questa mappa?');">
                <input type="hidden" name="idMappa" value="<%=mappa.getId()%>">
                <button class="buttonDel" type="submit">Cancella Mappa</button>
            </form>
        </div>

        <a href="MappeServlet">Torna alla lista delle mappe</a>

        <%
        } else {
        %>
        <p>Mappa non trovata.</p>
        <a href="MappeServlet">Torna alla lista delle mappe</a>
        <%
        }
        %>
